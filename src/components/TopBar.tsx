...
export function TopBar({ role, fullName }: { role: Role; fullName: string | null }) {
  ...
  return (
    <div className="flex items-center gap-3.5 px-4 md:px-[18px] py-2.5 bg-gradient-to-b from-[#171B21] to-bg border-b border-border sticky top-0 z-50 flex-wrap">
      <button onClick={() => router.back()} className="bg-surface3 border border-border rounded-md px-2.5 py-[7px] text-[12.5px] font-medium hover:bg-[#333d49]">
        ‹ Back
      </button>
      ...
      <Link href="/" className="bg-surface3 border border-border rounded-md px-2.5 py-[7px] text-[12.5px] font-medium hover:bg-[#333d49]">
        ⌂ Home
      </Link>
      ...
      <form
        action={signOut}
        onSubmit={(e) => {
          if (!window.confirm("Sign out of AurumTemple?")) e.preventDefault();
        }}
      >
